...
def get_distance(dist_tracks):
    distance= []
    for cell in np.unique(dist_tracks.cell.values):
        ps = np.nanmean(dist_tracks[dist_tracks.cell== cell].distance.values)
        distance.append(ps)
    distance = np.array(distance)
    distance = distance[distance<math.inf] 
    print('distance travelled histo calculated.')
    return distance
